...
function DJTab.build(parent, THEME, state, R, H)
	...
	-- ── SONG CARD VIRTUAL SCROLL (usa Card compartido) ──

	local function createSongCard()
		local c = Card.new(djSongsContainer, {
			size         = UDim2.new(1, -12, 0, 62),
			buttonIcon   = ICONS.PLAY_ADD,
			instanceName = "SongCard",
			visible      = false,
			zIndex       = 214,
		})

		-- Loading icon en el boton
		local loadingIcon = make("ImageLabel", {
			...
			Visible = false, Name = "LoadingIcon", Parent = c.actionBtn,
		})
		c._loadingIcon = loadingIcon

		-- Click handler
		c.actionBtn.MouseButton1Click:Connect(function()
			local songId = c.card:GetAttribute("SongID")
			if songId and not H.isInQueue(state.playQueue, songId) and not state.pendingCardSongIds[songId] then
				...
				c.actionBtn.BackgroundColor3 = THEME.elevated
				c.actionBtn.AutoButtonColor = false
	...
	local function getCardFromPool()
		for _, c in ipairs(djCardPool) do if not c.card.Visible then return c end end
		if #djCardPool < MAX_POOL then
			local c = createSongCard(); table.insert(djCardPool, c); return c
		end
	end

	local function releaseCard(cardObj)
		local idx = cardObj.card:GetAttribute("SongIndex")
		if idx then djCardsIndex[idx] = nil end
		cardObj.card.Visible = false
		cardObj.card:SetAttribute("SongIndex", nil); cardObj.card:SetAttribute("SongID", nil)
	end
	...
	local function releaseAllCards()
		...
		for _, c in ipairs(djCardPool) do
			...
			c.card:SetAttribute("SongIndex", nil); c.card:SetAttribute("SongID", nil)
		end
	end

	local function updateSongCard(cardObj, data, index)
		if not cardObj or not data then return end
		local card = cardObj.card
		card:SetAttribute("SongIndex", index); card:SetAttribute("SongID", data.id)
		djCardsIndex[index] = cardObj

		if state.selectedDJInfo and state.selectedDJInfo.cover then
			cardObj.imageLabel.Image = state.selectedDJInfo.cover
		end

		cardObj.nameLabel.Text = data.name or "Cargando..."
		cardObj.nameLabel.TextColor3 = data.loaded and THEME.text or THEME.muted
		cardObj.subtitleLabel.Text = data.artist or ("ID: " .. tostring(data.id))

		local ab = cardObj.actionBtn
		local icon = cardObj.actionIcon
		local loadingIcon = cardObj._loadingIcon
		local inQ = H.isInQueue(state.playQueue, data.id)
		local pending = state.pendingCardSongIds[data.id]
		local btnStroke = ab:FindFirstChildWhichIsA("UIStroke")

		if pending then
			ab.BackgroundTransparency = 1
			ab.AutoButtonColor = false
			if btnStroke then btnStroke.Transparency = 0 end
			icon.Visible = false
		...
		elseif inQ then
			...
			icon.Image = ICONS.CHECK; icon.ImageColor3 = Color3.new(1, 1, 1); icon.Visible = true
			if loadingIcon then loadingIcon.Visible = false end
		else
			ab.BackgroundTransparency = 1
			ab.AutoButtonColor = false
			if btnStroke then btnStroke.Transparency = 0 end
			icon.Image = ICONS.PLAY_ADD; icon.ImageColor3 = THEME.dim; icon.Visible = true
			if loadingIcon then loadingIcon.Visible = false end
		end
	...
	function api.updatePendingCard(response, songId, isSuccess)
		task.defer(function()
			...
			for _, cardObj in ipairs(djCardPool) do
				if cardObj.card.Visible and cardObj.card:GetAttribute("SongID") == songId then
					if cardObj._loadingIcon then cardObj._loadingIcon.Visible = false end
					cardObj.actionIcon.Visible = true

					local btnStroke = cardObj.actionBtn:FindFirstChildWhichIsA("UIStroke")
					if isSuccess then
						cardObj.actionIcon.Image = ICONS.CHECK
						cardObj.actionIcon.ImageColor3 = Color3.new(1, 1, 1)
						cardObj.actionBtn.BackgroundTransparency = 0
						cardObj.actionBtn.BackgroundColor3 = THEME.success
						if btnStroke then btnStroke.Transparency = 1 end
					else
						cardObj.actionIcon.Image = ICONS.PLAY_ADD
						cardObj.actionIcon.ImageColor3 = THEME.dim
						cardObj.actionBtn.BackgroundTransparency = 1
						if btnStroke then btnStroke.Transparency = 0 end
					end
					break
				end
	...
	function api.updateVisibleCards()
		...
		for idx, cardObj in pairs(djCardsIndex) do
			if cardObj and cardObj.card.Visible and (idx < first or idx > last) then releaseCard(cardObj) end
		end
