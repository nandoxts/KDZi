...
-- Módulos
local CentralPurchaseHandler = require(ServerScriptService.GiftManager.ManagerProcess)
...
local EventMessage  = ReplicatedStorage:WaitForChild("Commands"):WaitForChild("EventMessage")
...
local function broadcastLike(senderName, targetName, isSuperLike, amount)
	...
	if isSuperLike then
		EventMessage:FireAllClients(senderName .. " dio un Super Like (+" .. tostring(amount or SUPER_LIKE_VALUE) .. ") a " .. targetName .. "!", "superlike")
	else
		EventMessage:FireAllClients(senderName .. " dio un Like a " .. targetName .. "!", "like")
	end
	...
	if targetPlayer then
		if isSuperLike then
			GiveSuperLikeEvent:FireClient(targetPlayer, "LikeReceived", { IsSuperLike = true, Sender = senderName, Amount = amount or SUPER_LIKE_VALUE })
		else
			GiveLikeEvent:FireClient(targetPlayer, "LikeReceived", { IsSuperLike = false, Sender = senderName })
		end
	end
end
