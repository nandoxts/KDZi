...
local TIME_MULTIPLIERS = {
	VIP = {
		ID = Configuration.Gamepasses.VIP.id,
		MULTIPLIER = 2
	},
	COMMANDS = {
		ID = Configuration.Gamepasses.COMMANDS.id,
		MULTIPLIER = 5
	},
	ADMIN = {
		MULTIPLIER = 20
...
function TimePlayedTracker:UpdateMembershipCache(userId)
	...
	-- Verificar Admin (máxima prioridad)
	if AdminConfig:IsAdmin(player) then
		multiplier = TIME_MULTIPLIERS.ADMIN.MULTIPLIER
	end

	-- Solo verificar otros gamepasses si no es OWS
	if multiplier == 1 then
		-- Verificar COMMANDS
		if ShopManager.HasGamepass(player, TIME_MULTIPLIERS.COMMANDS.ID) then
			multiplier = TIME_MULTIPLIERS.COMMANDS.MULTIPLIER
			-- Verificar VIP
		elseif ShopManager.HasGamepass(player, TIME_MULTIPLIERS.VIP.ID) then
...
function TimePlayedTracker:SetupLeaderstats()
	local function createLeaderstats(player)
		task.spawn(function()
			...
			-- Crear stat de nivel en leaderstats para que el Overhead lo detecte
			if not leaderstatsFolder:FindFirstChild(self.LeaderstatsLevel) then
				local levelStat = Instance.new("IntValue")
				levelStat.Name = self.LeaderstatsLevel
				levelStat.Value = self:GetPlayerLevel(player.UserId)
				levelStat.Parent = leaderstatsFolder
			end
		end)
	end
