...
let topMessage8 = "";
// ================================
// AUTO-ENTRAR Y MODO OBSERVADOR
// ================================
let autoEnterRoomsEnabled = true;
let isInObserverMode = false;
let leaderboardDialogDetected = false;
// ================================
...
// CLICK HANDLER - MÁS AGRESIVO - SIEMPRE BUSCA EN CANVAS
document.addEventListener(
  ...
  function (e) {
    ...
    // IGNORAR clicks en inputs, textareas, buttons que no sean canvas
    ...
    if (candidates.length === 0) {
      console.log("[NO TEXT] Buffer vacío o sin métricas útiles");
      return;
    }
...
// Función para parsear atajos (100k → 100000, 1m → 1000000, etc)
function parseGoldShortcut(input) {
  input = String(input).trim().toLowerCase();
  const match = input.match(/^([\d.]+)([km]?)$/);
...
goldConverterOverlay.innerHTML = `
  <input type="text" id="golcAmount" placeholder="Ej: 100k, 1m, 500000" 
  ...
  <div style="display: flex; gap: 4px;">
    <button id="golc3m" style="flex: 1; padding: 6px; background: rgba(0, 0, 0, 0.5); color: #fff; border: 1px solid rgba(255, 255, 255, 0.2);
                             border-radius: 4px; cursor: pointer; font-size: 11px;">3M</button>
    <button id="golc4m" style="flex: 1; padding: 6px; background: rgba(0, 0, 0, 0.5); color: #fff; border: 1px solid rgba(255, 255, 255, 0.2);
                             border-radius: 4px; cursor: pointer; font-size: 11px;">4M</button>
  </div>
`;
...
document.getElementById("golcBtn").addEventListener("click", () => {
  ...
  // Intentar parsear con atajos (100k, 1m, etc)
  let amount = parseGoldShortcut(rawValue);

  // Si falla, intentar como número plano
  if (amount === null) {
...
document.getElementById("golc3m").addEventListener("click", () => {
  sendGoldAmount(3000000);
});

document.getElementById("golc4m").addEventListener("click", () => {
  sendGoldAmount(4000000);
});
...
// ================================
// MONITOR DE DIÁLOGO FINAL CON MUTATION OBSERVER
// ================================
let leaderboardObserver = null;

// Esperar a que exista el elemento antes de observar
function initLeaderboardMonitor() {
  const dialog = document.getElementById("finalLeaderboardDialog");

  if (!dialog) {
    console.log("⏳ Esperando elemento #finalLeaderboardDialog...");
    setTimeout(initLeaderboardMonitor, 500);
    return;
  }

  // Crear MutationObserver para detectar cambios de atributos (style, class, etc)
  leaderboardObserver = new MutationObserver(() => {
    const computedStyle = window.getComputedStyle(dialog);
    const inlineStyle = dialog.getAttribute("style") || "";
    const isVisible =
      computedStyle.display !== "none" && !dialog.classList.contains("hidden");
    // Si está visible y no hemos activado todavía
    if (isVisible && !leaderboardDialogDetected) {
      leaderboardDialogDetected = true;
      console.log("📊 ¡DIÁLOGO FINAL VISIBLE! - Presionando Kapat...");

      if (autoEnterRoomsEnabled) {
        const closeBtn = document.getElementById("finalLeaderboardDialog_ok");
        if (closeBtn) {
          closeBtn.click();
          console.log("✅ Botón Kapat clickeado");
          ...
          setTimeout(() => {
            ...
            if (playBtn) {
              ...
              console.warn("⚠️ Botón playBtn no encontrado");
              onClickPlay();
            }
            console.log("✅ Auto-entrada ejecutada");
          }, 5000);
        } else {
          console.warn(
            "⚠️ Botón Kapat (#finalLeaderboardDialog_ok) no encontrado",
          );
          setTimeout(() => {
            const playBtn = document.getElementById("playBtn");
            if (playBtn) {
              playBtn.click();
              console.log("✅ Botón Play clickeado (fallback)");
            } else {
              console.warn("⚠️ Botón playBtn no encontrado");
              onClickPlay();
            }
    ...
    // Si está oculto, resetear el flag
    if (!isVisible && leaderboardDialogDetected) {
      leaderboardDialogDetected = false;
      console.log("🔄 Diálogo oculto - Flag reseteado");
    }
  });

  // Observar cambios en atributos style y class
  leaderboardObserver.observe(dialog, {
    attributes: true,
    attributeFilter: ["style", "class"],
    attributeOldValue: true,
  });

  console.log("👁️ Monitor de Diálogo ACTIVO");
}

// Iniciar el monitor cuando el DOM esté listo
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", initLeaderboardMonitor);
} else {
  initLeaderboardMonitor();
}
